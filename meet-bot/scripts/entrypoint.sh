...
rm -f /tmp/pulse-*/pid /run/user/*/pulse/pid 2>/dev/null || true
sleep 0.5

# Start PulseAudio with retries
for i in 1 2 3; do
  pulseaudio --daemonize --no-cpu-limit --disable-shm=true --exit-idle-time=-1 \
    --system=false --log-level=warning 2>/dev/null && break
  echo "[Entrypoint] PulseAudio attempt $i failed, retrying..."
  sleep 1
done
sleep 1
...
if ! pactl info >/dev/null 2>&1; then
  echo "[Entrypoint] WARNING: PulseAudio not running, trying system mode..."
  pulseaudio --daemonize --no-cpu-limit --disable-shm=true --exit-idle-time=-1 \
    --system=true --log-level=warning 2>/dev/null || true
  sleep 1
fi
...
pactl load-module module-null-sink sink_name=tts_output sink_properties=device.description="TTS_Output" || true

# Create a virtual source from tts_output's monitor so Chrome picks it up as a mic
pactl load-module module-remap-source master=tts_output.monitor source_name=virtual_mic source_properties=device.description="Virtual_Mic" || true

# Meet capture sink — we route Meet's audio output here so we can record it
pactl load-module module-null-sink sink_name=meet_capture sink_properties=device.description="Meet_Capture" || true

# Set defaults
pactl set-default-sink meet_capture 2>/dev/null || true
